...
$ExposureOverrides = @{'appendicitis|20123918'='opened for source-schema inspection; removed from final test'}
...
$records=@()
foreach($file in Get-ChildItem -LiteralPath $SourceDir -Filter '*.json' -File | Where-Object {$_.Name -ne 'manifest.json' -and -not $_.Name.StartsWith('._')} | Sort-Object Name){
...
foreach($disease in $Diseases){
    ...
    for($i=0;$i -lt $group.Count;$i++){$overrideKey="$disease|$($group[$i].hadm_id)";$overrideReason=$(if($ExposureOverrides.ContainsKey($overrideKey)){$ExposureOverrides[$overrideKey]}else{$null});$isTest=$i-lt$nTest-and$null-eq$overrideReason;$patients += [pscustomobject][ordered]@{disease=$group[$i].disease;hadm_id=$group[$i].hadm_id;n_steps=$group[$i].n_steps;source_path=$group[$i].source_path;split_hash=$group[$i].split_hash;partition=$(if($isTest){'final_test'}else{'development'});annotation_access=$(if($isTest){'metadata_only_until_framework_and_models_frozen'}elseif($overrideReason){'development_allowed_after_exposure_override'}else{'development_allowed'});development_override_reason=$overrideReason}}
}
$summary=[ordered]@{}
...
$split=[ordered]@{schema_version='1.0.0';created_from='results/annotation_experiment/full/*.json';eligibility_rule='object with disease, hadm_id, steps; manifest.json and AppleDouble ._* metadata files excluded';algorithm='within disease sort SHA-256(salt|disease|hadm_id); assign round(0.20*n) lowest hashes to final_test; any explicitly recorded exposure override is moved to development without replacement';hash_salt=$SplitSalt;exposed_development_overrides=@([ordered]@{disease='appendicitis';hadm_id=20123918;reason=$ExposureOverrides['appendicitis|20123918']});unit='patient trajectory; all steps inherit the patient partition';final_test_policy='only identity, disease, source path, and step-count metadata are recorded; annotation content may not be profiled, selected, coded, or used for revision before freeze';summary=$summary;patients=@($patients|Sort-Object disease,hadm_id)}
...
$cumA=New-Object 'Collections.Generic.HashSet[string]';$cumQ=New-Object 'Collections.Generic.HashSet[string]';$cumR=New-Object 'Collections.Generic.HashSet[string]';$rounds=@();foreach($name in $batches.Keys){$newA=@($batchCodes[$name].assumption|Where-Object{-not$cumA.Contains($_)});$newQ=@($batchCodes[$name].question|Where-Object{-not$cumQ.Contains($_)});$newR=@($batchCodes[$name].requirements|Where-Object{-not$cumR.Contains($_)});foreach($v in $batchCodes[$name].assumption){[void]$cumA.Add($v)};foreach($v in $batchCodes[$name].question){[void]$cumQ.Add($v)};foreach($v in $batchCodes[$name].requirements){[void]$cumR.Add($v)};$rounds += [pscustomobject][ordered]@{batch=$name;n_patients=$batches[$name].Count;new_top_level_assumption_candidates=$newA;new_top_level_question_candidates=$newQ;new_answer_requirement_candidates=$newR;material_schema_change=($name-eq'initial_24');review_result=$(if($name-eq'initial_24'){'first formal draft established from deterministic paired-view candidate coding'}else{'no new lexical top-level family; this is not an independent qualitative review'})}}
$nScaffold=@($rows|Where-Object{$_.view_comparison.possible_scaffold_induction}).Count
$sat=[ordered]@{schema_version='1.0.0-development';scope='top-level A/Q types and recurrent answer-requirement dimensions';rounds=$rounds;conclusion='lexically_stable_but_not_yet_qualitatively_saturated';basis="two fresh non-overlapping 12-patient development batches produced no new candidates under the same deterministic lexical rules; $nScaffold of $($rows.Count) steps gained at least one candidate only after schema-light fields were exposed, so independent review is required before a saturation or freeze claim";limits=@('lexical candidates require independent human/clinical framework review','does not establish prevalence, correctness, inter-rater reliability, or final-test transport','final-test annotation content remained excluded from discovery','absence of a new regex hit is not evidence that no new conceptual structure exists','paired-view scaffold dependence remains high and must be adjudicated');next_gate='independent dual-route framework check and manual/clinical adjudication on fresh unused development patients; do not open final test'};Write-Json (Join-Path $OutDir 'saturation_audit.json') $sat
